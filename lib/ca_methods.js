...
exports.methods = {
    ...
    async checkClientInfo() {
        try {
            ...
            if (!this.state.clientInfo ||
                ...
                this.$.log &&
                    this.$.log.debug('checkClientInfo: Retrying: ' +
    ...
    async handleNewClientInfo(topic, info, from) {
        var callerOwner = json_rpc.splitName(from)[0];
        if ((!this.state.isManager) ||
            ...
            var $$ = this.$.sharing.$;
            $$.pubKeysMaster.set(from, JSON.parse(info));
            return [];
        }
    },

    // topic is this.$.pubsub.FORUM_PREFIX + me + NEW_MSG_SUFFIX;
    async handleNewMessage(topic, msg, from) {
        if (this.state.isManager) {
    ...
    async handlePairingRequest(topic, msg, from) {
    ...
    async handlePairingAck(topic, msg, from) {
